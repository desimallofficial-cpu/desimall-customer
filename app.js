...
const DesiMallApp = {
  ...
  updateUserUI() { const link=document.getElementById('userAuthLink');const user=this.safeJSON('desimall_user',null);if(!link)return;if(!user){link.href='pages/login.html';link.innerHTML='<i class="fa-regular fa-user"></i><span>Login</span>';return;}const name=user.Name||user.FullName||user.name||'Profile';link.href='pages/profile.html';link.title='My Profile';link.innerHTML=`<i class="fa-solid fa-circle-user"></i><span>${this.text(String(name).split(' ')[0])}</span>`; },
